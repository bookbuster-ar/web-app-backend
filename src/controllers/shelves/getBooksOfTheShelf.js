...
const getBooksOfTheShelf = async (book_shelf_category_id) => {
  ...
  const books = shelfBooks.books?.map((field) => {
    const coverImage =
      field.images && field.images.length > 0 ? field.images[0].image : null;
    const extraImages =
      field.images && field.images.length > 1
        ? field.images.slice(1).map((img) => img.image)
        : [];
    ...
    return {
      ...
      editorial_collection: field.editorial_collection
        ? field.editorial_collection.name
        : '',
...
module.exports = getBooksOfTheShelf;
