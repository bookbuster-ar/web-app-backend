const { BookShelves, BookShelfCategory, BookShelfCategoryInterm, Book } = require('../../models')
...
const getShelfWithBooks = async (userid) => {
    ...
    const formattedBookShelfCategories = plainShelveAndShelfs.book_shelf_categories.map(category => {
        const formattedBooks = category.books.map(book => {
            const coverImage = book.images && book.images.length > 0 ? book.images[0].image : null;
            const extraImages = book.images && book.images.length > 1 ? book.images.slice(1).map(img => img.image) : [];
            ...
            return {
                ...
                editorial_collection: book.editorial_collection ? book.editorial_collection.name : '',
